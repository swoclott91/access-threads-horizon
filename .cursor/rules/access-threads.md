...
- **AT mega panel `top`:** Horizon’s mega menus (`blocks/_header-menu.liquid`) use `position: absolute` on `.menu-list__submenu` with `top: calc(100% - 1px + …)` from the **nav list item**, not the bottom of `#header-component`. AT keeps `position: fixed` for full-width bleed but should set `top` from **`.header__row--top`’s `getBoundingClientRect().bottom`**, with scroll/resize/`ResizeObserver` while open — not the whole header component box (avoids a gap above the panel).
