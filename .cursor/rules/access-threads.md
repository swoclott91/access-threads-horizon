...
- **AT mega panel `top`:** Do not anchor only to `.header__row--top` — **menu row** can be `.header__row--bottom` (section `menu_row`). Use **`trigger.closest('.header__row')`**. Anchor **`top` to that row’s `getBoundingClientRect().bottom`** (the row `border-bottom` seam), not `min(.at-menu__nav, …)` — the short nav box left a hero-coloured band between the border and the panel. Overlap seam by **`max(2, 1 + borderBottomWidth)`** px (Horizon mega menus use `100% - 1px + var(--border-bottom-width)`). `ResizeObserver` + scroll/resize on `#header-component`. Dropdown **`z-index: calc(var(--layer-header-menu) + 1)`** so it paints above the row’s open-state stacking.
