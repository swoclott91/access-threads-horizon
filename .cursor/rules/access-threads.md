...
- **AT mega panel `top`:** Do not anchor only to `.header__row--top` — **menu row** can be `.header__row--bottom` (section `menu_row`). **Do not use `row.bottom` alone:** the row is often taller than `.at-menu__nav`, which pushed `top` far too low (e.g. ~107px vs ~66px). Use **`Math.min(nav.bottom, row.bottom, trigger.bottom)`** for the anchor Y, then subtract **`max(2, 1 + row borderBottomWidth)`** for seam overlap. `ResizeObserver` + scroll/resize on `#header-component`. Dropdown **`z-index: calc(var(--layer-header-menu) + 1)`**.
