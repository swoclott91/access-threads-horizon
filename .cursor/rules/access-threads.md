...
- **AT mega panel `top`:** Do not anchor only to `.header__row--top` — **menu row** can be `.header__row--bottom` (section `menu_row`). Use **`trigger.closest('.header__row')`**. Also **`.at-menu__nav`** is often shorter than the row; use **`min(nav.getBoundingClientRect().bottom, row.getBoundingClientRect().bottom)`** so the panel meets the link strip. `ResizeObserver` on `#header-component`; subtract ~1px for seam parity with Horizon mega menus.
